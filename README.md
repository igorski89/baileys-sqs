...
```json
{"type":"command_ack","payload":{"correlation_id":"...","command_type":"send_text","to":"1234567890@s.whatsapp.net","from":"18005551234:1@s.whatsapp.net","me_lid":"123456789012345@lid","ok":true,"message_id":"3EB0..."}}
```

`message_id` is the real WhatsApp message id (`null` for commands that don't produce one — currently `send_presence` and `send_read_receipt`). `from` is baileys-sqs's own WhatsApp id (`sock.user.id`, unnormalized) at the time the command was processed. `me_lid` is baileys-sqs's own LID-form WhatsApp identity (`sock.user.lid`), alongside the phone-number-form `from` — `null` if the account has no LID identity yet assigned. On failure, `ok` is `false` and `error` is set instead of `message_id`. `send_poll`'s ack additionally includes `message_secret` (base64) — the poll's encryption secret, needed to decrypt votes later; baileys-sqs never stores it. Without `correlation_id`, behavior is unchanged from before — no `command_ack` is emitted.
...
baileys-sqs handles this automatically, no configuration needed: it keeps a per-chat cache of the last-known disappearing-messages duration, populated both when the setting changes (`chats.update`) and passively from every incoming message's own metadata, then applies it to every `send_text`/`send_media`/`send_poll`/`raw` send for that chat unless the command already specifies its own `options.ephemeralExpiration`.

The cache is persisted as `ephemeral-cache.json` inside `SESSION_DIR` — the same directory already used for auth/session data — written through on every real change (not periodically; chat-setting changes are rare) and loaded once at startup. A chat that stays quiet immediately after a restart still keeps its previously-known setting, instead of being treated as non-disappearing until new activity reveals it again. If the file is missing or corrupt, the cache just starts empty rather than failing startup — the self-healing behavior below still applies from there.

For any chat the cache hasn't seen yet (a fresh deployment, or a chat that's genuinely never been observed), it self-heals as soon as any message flows through that chat (which reveals its current setting) — there's no way to eagerly query a 1:1 chat's setting on demand (no query API exists, and Baileys' history-sync, which does include this data, normally only fires on a fresh pairing, not a routine reconnect of an already-linked session).
